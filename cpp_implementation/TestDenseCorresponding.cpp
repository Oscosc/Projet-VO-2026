...
#define DEFAULT_DISPARITY_FACTOR 1

int main(int argc, char** argv) 
{
    if (argc < 3 || argc > 9) {
        std::cerr << "Usage : " << argv[0] << " <path_to_left_image> <path_to_right_image> "
            << "<optional:patch_size> <optional:max_disparity> <optional:disparity_factor> "
    ...
    const int disparityFactor   = (argc >= 6) ? std::stoi(argv[5]) : DEFAULT_DISPARITY_FACTOR;
    const char* checkDisparity  = (argc >= 7) ? argv[6] : "true";
    const char* outputPathLeft  = (argc >= 8) ? argv[7] : "left_disparity_output.png";
    const char* outputPathRight = (argc == 9) ? argv[8] : "right_disparity_output.png";

    Weyl::Image::Image imageLeft, imageRight, rawDispLeft, rawDispRight, outputLeft, outputRight;
    ...
    std::cout << "  -> Maps factor:   " << disparityFactor << std::endl;
    ...
    std::cout << "[PERFORMANCE] Dense corresponding execution time : " << execTime.count() << " ms\n";

    if(std::strcmp(checkDisparity, "true") == 0) {
        start = std::chrono::high_resolution_clock::now();
        Weyl::LeftRightConsistency(rawDispLeft, rawDispRight, outputLeft, outputRight);
        end = std::chrono::high_resolution_clock::now();
        execTime = end - start;
        std::cout << "[PERFORMANCE] Left-Right consistency check execution time : " << execTime.count() << " ms\n";
    }
    else {
        outputLeft = rawDispLeft;
        outputRight = rawDispRight;
    }
    ...
    Weyl::Image::ScaleImageData(outputLeft, disparityFactor);
    Weyl::Image::ScaleImageData(outputRight, disparityFactor);
